...
peer_generate_metrics(routingtables_t *rt, collector_t *c, perpeer_info_t *p)
{
  ...
  p->kp_idxs.pfx_withdrawal_cnt_idx = \
    add_p_metric(p->kp, rt->metric_prefix, c->collector_str, p->peer_str, "peer_pfx_withdrawal_cnt");
...
routingtables_dump_metrics(routingtables_t *rt, uint32_t time_now)
{
  ...
  fprintf(stderr, "PROCESSING TIME: %"PRIu32"\n", processing_time);  
          ...
          if(c->state != ROUTINGTABLES_COLLECTOR_STATE_UNKNOWN)
            {
              ...
              /* @todo collect metrics and set their value here */
