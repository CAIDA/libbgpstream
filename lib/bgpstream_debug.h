...
#define bgpstream_check(A, M, ...) if(!(A)) { log_err(M, ##__VA_ARGS__); errno=0; goto error; }
...
#define bgpstream_sentinel(M, ...)  { log_err(M, ##__VA_ARGS__); errno=0; goto error; }

// check_mem that makes sure a pointer is valid, and if it isn't reports it as an error with "Out of memory."
#define bgpstream_check_mem(A) check((A), "Out of memory.")
...
#define bgpstream_check_debug(A, M, ...) if(!(A)) { debug(M, ##__VA_ARGS__); errno=0; goto error; }
