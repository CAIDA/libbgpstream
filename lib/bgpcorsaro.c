...
static inline void bgpcorsaro_record_state_reset(bgpcorsaro_record_t *record)
{
  ...
  bgpcorsaro_tag_state_reset(&record->state);

#ifdef WITH_PLUGIN_IPMETA
  /* reset the matched ipmeta records */
  for(i=0; i<IPMETA_PROVIDER_MAX; i++)
    {
      record->state.ipmeta_records[i] = NULL;
    }
  /* reset the default match */
  record->state.ipmeta_record_default = NULL;
#endif
}
...
static void bgpcorsaro_free(bgpcorsaro_t *bgpcorsaro)
{
  ...
  if(bgpcorsaro->tag_manager != NULL)
    {
      bgpcorsaro_tag_manager_free(bgpcorsaro->tag_manager);
      bgpcorsaro->tag_manager = NULL;
    }
...
static bgpcorsaro_t *bgpcorsaro_init(char *template)
{
  ...
  if((e->tag_manager = bgpcorsaro_tag_manager_init(e)) == NULL)
    {
      bgpcorsaro_log(__func__, e, "could not initialize tag manager");
      goto err;
    }
