...
static char *build_filename(bgpstream_mysql_datasource_t *ds)
{
  ...
  // default path
  if (sprintf(filename, "%s/%s/%s/%s/%s/%s.%s.%s.%d.%s", ds->mysql_dump_path,
  ...
  bgpstream_mysql_datasource_t *mysql_ds, bgpstream_input_mgr_t *input_mgr)
{
  ...
  */
  ...
  while (stmt_ret != 0) {
    ...
    fprintf(stderr, "bgpstream: connection to mysql failed, retrying [last "
                    "timestamp: %ld, attempt: %d]\n",
        ...
        if (mysql_query(mysql_ds->mysql_con, "set time_zone='+0:0'") == 0) {
          if ((mysql_ds->stmt = mysql_stmt_init(mysql_ds->mysql_con)) != NULL) {
            if (mysql_stmt_prepare(mysql_ds->stmt, mysql_ds->sql_query,
              ...
              if (mysql_stmt_bind_param(mysql_ds->stmt, mysql_ds->parameters) ==
                  0) {
                if (mysql_stmt_bind_result(mysql_ds->stmt, mysql_ds->results) ==
                    0) {
  ...
  while (mysql_stmt_fetch(mysql_ds->stmt) == 0) {
    ...
    bgpstream_debug("\t\tBSDS_MYSQL: added %d new inputs to input queue",
                    num_results);
    bgpstream_debug("\t\tBSDS_MYSQL: %s - %s - %d", filename,
                    mysql_ds->type_name_res, mysql_ds->filetime_res);
    // here
